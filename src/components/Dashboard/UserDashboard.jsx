...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    const loadUserData = async () => {
      ...
      console.log(
        "API URL:",
        process.env.NEXT_PUBLIC_API_URL || "http://server.nybff.us",
      );
          ...
          <div className="col-span-12 md:col-span-9 space-y-6">
            ...
            >
              {[
                ...
                {
                  label: "Selections",
                  value: user?.stats?.selections || 0,
                  icon: "🏆",
                  type: "selections",
                },
                {
                  label: "Awards",
                  value: user?.stats?.awards || 0,
                  icon: "⭐",
                  type: "awards",
                },
              ].map((stat, idx) => (
                <div
                  key={idx}
                  className="bg-linear-to-br from-gray-900 to-gray-950 rounded-2xl p-5 border border-gray-800 hover:border-gray-700 transition-all duration-300 hover:shadow-xl"
                >
                  <div className="flex items-center justify-between mb-3">
                    <div
                      className={`w-10 h-10 bg-linear-to-br ${getGradient(stat.type)} rounded-xl flex items-center justify-center text-xl shadow-lg`}
                    >
                      {stat.icon}
                    </div>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 bg-[#1EB97A]/20 rounded-full flex items-center justify-center">
                        <svg
                          className="w-3.5 h-3.5 text-[#1EB97A]"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                    ...
                    <Link
                      href="/settings"
                      className="text-xs text-[#1EB97A] hover:text-emerald-400 font-medium flex items-center gap-1"
                    >
                      Edit Profile
                      <svg
                        className="w-3 h-3"
